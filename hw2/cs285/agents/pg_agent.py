...
class PGAgent(BaseAgent):
    ...
    def calculate_q_vals(self, rewards_list):
        ...
        if not self.reward_to_go:
            q_values=np.concatenate([self._discounted_return(rew) for rew in rewards_list])

        # Case 2: reward-to-go PG
        # Estimate Q^{pi}(s_t, a_t) by the discounted sum of rewards starting from t
        else:
            q_values=np.concatenate([self._discounted_cumsum(rew) for rew in rewards_list])
        ...
        if self.nn_baseline:
            ...
            if self.gae_lambda is not None:
                ...
                for i in reversed(range(batch_size)):
                    ...
                    #if terminals[i]!=1:
                        re=rews[i]
                        advantages[i]+=(re+self.gamma*values[i+1]-values[i])
    ...
    def _discounted_return(self, rewards):
        ...
        for i in range(len(rewards)):
            sum_val+=self.gamma**i*(rewards[i])

        list_of_discounted_returns=np.repeat(sum_val,len(rewards))
    ...
    def _discounted_cumsum(self, rewards):
        ...
        for i in range(T):
            time_spilt=np.arange(0,T)
            gamma_list=self.gamma**(time_spilt-i)
            sum_val=np.sum(gamma_list[i:]*rewards[i:])
            list_of_discounted_cumsums.append(sum_val)
