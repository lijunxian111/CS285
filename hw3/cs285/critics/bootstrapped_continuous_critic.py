...
class BootstrappedContinuousCritic(nn.Module, BaseCritic):
    ...
    def update(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        ...
        ob_no, next_ob_no, reward_n, terminal_n = \
            [x.to(self.device) for x in [ob_no, next_ob_no, reward_n, terminal_n]]
        for _ in range(self.num_target_updates):
             target_val=reward_n+self.gamma*self.forward(next_ob_no)*torch.logical_not(terminal_n)
             for _ in range(self.num_grad_steps_per_target_update):
                  self.optimizer.zero_grad()
                  loss=self.loss(self.critic_network(ob_no).squeeze(),target_val)
                  loss.backward()
                  self.optimizer.step()
