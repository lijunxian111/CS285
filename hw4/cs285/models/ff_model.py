...
class FFModel(nn.Module, BaseModel):
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        """
        prediction = self.forward(obs,acs,**data_statistics)[0]# TODO(Q1) get the predicted next-states (s_t+1) as a numpy array
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        :return:
        """
        #target = ((next_observations-observations)-ptu.from_numpy(data_statistics['delta_mean']))/ptu.from_numpy(data_statistics['delta_std'])#
        ...
        loss = self.loss(ptu.from_numpy(target),self.forward(observations,actions,**data_statistics)[1])# TODO(Q1) compute the loss
        # Hint: `self(...)` returns a tuple, but you only need to use one of the
        # outputs.
